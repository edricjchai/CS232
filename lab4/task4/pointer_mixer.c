...
void strlen_vs_sizeof() {
    ...
    printf("strlen(str):%d sizeof(str):%d sizeof(str2):%d sizeof(s):%d, sizeof(*s):%d\n",
         (int) strlen(str),  //the length of the str 6
         (int) sizeof(str),  //the memory size of the str 7
         (int) sizeof(str2),  //the memory size of the str2 8
         (int) sizeof(s),    //the memory size of a pointer 4
         (int) sizeof(*s)    //the memory size of a char 1
         );
...
*/
    ...
    printf("*(*(darray+2)+2) = %d\n", *(*(darray+2)+2)); //*(*(darray+2)+2) = 11
    //double array is double pointer
    printf("     darray[2][2] = %d\n", darray[2][2]); //darray[2][2] = 11
}
// ----------------------------------------------------------------------------
void string_double_array_pointer_array() {
    ...
    printf("str1:%p\n",str1); //lower, initialized first
    printf("str2:%p\n",str2); //which is at the higher address? why?
                              //check the memory layout of your process
                              //what lays at the bottom?
                              //Text lays at the bottom of the memory layout
                              /*
                              *-----------
                              *Environment
                              *Stack
    ...
    for(i=0;i<4;i++){
      printf("strings[%d]: '%s' %p\n",i,strings[i],strings[i]);  //the earlier the string in the array the lower the address, strings[0] should be the lowest in the array and strings[3] the highest
...
void string_equal() {
    ...
    char * s3 = "Pace";
    char * s4 = "Pace";
    if(s1 == s2){ //false, different addresses. Getting char addresses not the strings themselves
        printf("Go Pace!\n");
    }else if(s3 == s4) { //true, outputs: BEAT CUNY!
        printf("Beat CUNY!\n");
    }else {
        printf("Crash SUNY!\n");
    }
    printf("\n");
    printf("s1: %p == s2: %p? \n", s1, s2); //different places in memory, since these are not pointers so they aren't accurate
    printf("s3: %p == s4: %p? \n", s3, s4); //similar in memory, since pointer variables looks directly at memory
}
...
void trace_pointers() {
    ...
    int b = 7;
    int c = 999;
    int *t = &a; //pointing to a
    int *u = NULL;
    printf("%d %d\n", a, *t); //42 42

    c = b; //assignment change c to 7
    u = t; //u pointing to t
    printf("%d %d\n", c, *u); //7 42
    ...
    //*u pointing to *t which points to a
    *t = 123; //points to a, therefore changes a to 123
...
{
    *(value.a) = 2; //Cause it is a pointer it changes to 2
    value.b = 3; //does not change outside of foo because does not have a pointer(->)
}
...
void trace_structs_pointers()
{
    ...
    my_stuff.a = &temp; //points to temp, temp = 0
    my_stuff.b = 1; //value of b is 1
...
void readonly_vs_stack() {
  ...
  str1[0] = 't';
  printf("str1: %s \n",str1); //str1: This is a locust tree
  str2[0] = 't';
  printf("str2: %s \n",str2); //seg. fault, trying to print something out of memory, program crashes
}
